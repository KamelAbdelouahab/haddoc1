...
def genCaph_CNN(network, caph_net_filename, caph_dataype, acteurconv, shiftnorm):
    ...
    for layer in list(Blobs.keys()):
        if 'conv' in layer:
            if 'conv1' in layer:
                generateFirstLayer(caph_net_filename, Params[layer][0].data, "i", acteurconv, name_weights + str(
                    layer), shiftnorm, name_biais + str(layer), name_wire_relu, name_wire + str(layer))
                name_wire_previous = name_wire_relu
            else:
                generateConvLayer(caph_net_filename, Params[layer][0].data, name_wire_previous, acteurconv, name_weights + str(
                    layer), shiftnorm, name_biais + str(layer), name_wire + str(layer), Params[layer][0].data.shape[1])
        if 'pool' in layer:
            generatePoolingLayer(caph_net_filename, name_wire_previous,
                                 Params[layer_previous][0].data.shape[0], name_wire + str(layer), 'pool')
...
def genInterfaces(network, caph_net_filename, caph_dataype, C2V_CPP_LIB, C2V_DIRNAME):
    pass
...
def genCaph_FC(network, caph_net_filename, caph_dataype, C2V_CPP_LIB, C2V_DIRNAME):
    ...
    for layer in list(Blobs.keys()):
        if final_layer in layer:
            ...
            nx_feat = network.blobs[final_layer].data.shape[2]
            ny_feat = network.blobs[final_layer].data.shape[3]
            nfeat = network.blobs[final_layer].data.shape[1]
            fc_wirename_in = final_layer
            nb_unit_fc = 10
            nbclass = 10
            fc_wirename_out = "w_fc"
    ...
    for nb in range(sizesum3):
        f.write("stream w_" + final_layer + "%d : " % nb)
        f.write("%s dc " % caph_dataype)
        f.write("to " + "\"w_" + final_layer + "%d.txt\";\n" % nb)
    f.close()
    ...
    print(subprocess.Popen(C2V_CPP_LIB + "/gen_cnn_code %d %d %d %d %d %d %d %s %s %d %s %s" % (repsize, sizesum1,
                                                                                                sizesum2,
                                                                                                nfeat,
                                                                                                nx_feat, ny_feat,
                                                                                                nb_unit_fc, fc_wirename_in, fc_wirename_out, nbclass, "y_", datatype), shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT).stdout.read())
    os.environ["GLOG_minloglevel"] = "0"
    print(subprocess.Popen("cp " + "/home/kamel/dev/haddoc1/script/lib/caph/* " +
    ...
    print(subprocess.Popen("mv " + "/home/kamel/dev/haddoc1/cnn/* " +
                           "/home/kamel/dev/haddoc1/caph_generated",
                           shell=True,
                           stdout=subprocess.PIPE,
                           stderr=subprocess.STDOUT).stdout.read())
    print(subprocess.Popen("mv " + "/home/kamel/dev/haddoc1/utils/* " +
                           "/home/kamel/dev/haddoc1/caph_generated",
                           shell=True,
                           stdout=subprocess.PIPE,
                           stderr=subprocess.STDOUT).stdout.read())
...
def generateFirstLayer(caph_net_filename, filters_conv1, name_wire_input, acteurconv, name_weights_conv1, shiftnorm, name_biais_conv1, name_wire_relu, name_wire_output_conv1):
    ...
    f.close()
    generateFactLayer(caph_net_filename, name_wire_output_conv1,
                      filters_conv1.shape[0], name_wire_relu, 'relu')
...
def generateConvLayer(caph_net_filename, filters_conv2, name_wire_in, acteurconv, weights, shiftnorm, biais, name_wire_out, nconnect):
    with open(caph_net_filename, "a") as f:
        ...
        f.write("\n net(")
        # recuperer le nombre de kernels
        for nb in range(filters_conv2.shape[0]):
        ...
        for n in range(filters_conv2.shape[0]):  # sur chaque neurone
            f.write("\t(")
            # recuperer dentres de chaque neurone
            for nb in range(filters_conv2.shape[1]):
                # recuperer le nom des entres de la couche precedente
